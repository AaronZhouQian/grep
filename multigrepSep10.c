...
void * grep_thread_routine(void *arg_pt);

static void dos_binary (void);
static void dos_unix_byte_offsets (void);
static size_t undossify_input (char *, size_t, char *);
...
static bool
reset (int fd, struct stat const *st, struct grep_info *info)
{
    char **bufbeg_local = &(info->bufbeg);
    char **buflim_local = &(info->buflim);
    *bufbeg_local = *buflim_local = ALIGN_TO (buffer + 1, pagesize);
    (*bufbeg_local)[-1] = eolbyte;
    bufdesc = fd;
...
fillbuf (size_t save, struct stat const *st, struct grep_info *info)
{
    ...
    size_t saved_offset = *buflim_local - save - buffer;
    
    if (pagesize <= buffer + bufalloc - sizeof (uword) - *buflim_local)
    {
        readbuf = *buflim_local;
        *bufbeg_local = *buflim_local - save;
    }
    else
    {
        ...
        *bufbeg_local = readbuf - save;
        memmove (*bufbeg_local, buffer + saved_offset, save);
        (*bufbeg_local)[-1] = eolbyte;
        if (newbuf != buffer)
        {
            free (buffer);
    ...
    fillsize = undossify_input (readbuf, fillsize, *bufbeg_local);
    *buflim_local = readbuf + fillsize;
    ...
    memset (*buflim_local, 0, sizeof (uword));
    
    /* Mark the part of the buffer not filled by the read or set by
     the above memset call as ASAN-poisoned.  */
    asan_poison (*buflim_local + sizeof (uword),
                 bufalloc - (*buflim_local - buffer) - sizeof (uword));
...
static void
nlscan (char const *lim, struct grep_info *info)
{
    uintmax_t *totalnl_local = &(info->totalnl);
    size_t newlines = 0;
    char const *beg;
...
static void
print_filename_in_thread (char const *fname)
{
    pr_sgr_start_if (filename_color);
    fputs_errno (fname);
    pr_sgr_end_if (filename_color);
...
print_line_head (char *beg, size_t len, char const *lim, char sep, struct grep_info *info)
{
    ...
    if (out_byte)
    {
        uintmax_t pos = add_count (info->totalcc, beg - *bufbeg_local);
         ...
         cur = b + match_size)
    {
        ...
        else
        {
            ...
            if (only_matching)
            {
                ...
                if (! print_line_head (b, match_size, lim, sep, info))
                    return NULL;
            }
            else
            {
...
static void
prline (char *beg, char *lim, char sep, struct grep_info *info)
{
    ...
    if (!only_matching)
        if (! print_line_head (beg, lim - beg - 1, lim, sep, info))
            return;
        ...
        || (color_option && (*line_color || *match_color)))
    {
        ...
        {
            beg = print_line_middle (beg, lim, line_color, match_color, info);
            if (! beg)
                return;
        }
...
static void
prtext (char *beg, char *lim, struct grep_info *info)
{
    intmax_t *outleft_local =&(info->outleft);
    char ** lastout_local = &(info->lastout);
    ...
    if (!*out_quiet_local)
    {
        ...
        char const *bp = *lastout_local ? *lastout_local : *bufbeg_local;
        intmax_t i;
        ...
        while (p < beg)
        {
            ...
            nl++;
            prline (p, nl, SEP_CHAR_REJECTED, info);
            p = nl;
        }
    }
    ...
    if (out_invert)
    {
        ...
        for (n = 0; p < lim && n < *outleft_local; n++)
        {
            ...
            nl++;
            if (!*out_quiet_local)
                prline (p, nl, SEP_CHAR_SELECTED, info);
            p = nl;
        }
    }
    else
    {
        ...
        if (!*out_quiet_local)
            prline (beg, lim, SEP_CHAR_SELECTED, info);
        n = 1;
        p = lim;
    }
    
    *after_last_match_local = bufoffset - (*buflim_local - p);
...
static intmax_t
grepbuf (char *beg, char const *lim, struct grep_info *info)
{
...
static intmax_t
grep (int fd, struct stat const *st, struct grep_info *info)
{
    intmax_t nlines, i;
    size_t residue, save;
    char oldc;
    ...
    for (bool firsttime = true; ; firsttime = false)
    {
        if (nlines_first_null < 0 && eol && binary_files != TEXT_BINARY_FILES
            && (buf_has_nulls (*bufbeg_local, *buflim_local - *bufbeg_local)
                || (firsttime && file_must_have_nulls (*buflim_local - *bufbeg_local, fd, st))))
        {
        ...
        lastnl = *bufbeg_local;
        if (info->lastout)
            info->lastout = *bufbeg_local;
        
        beg = *bufbeg_local + save;
        
        /* no more data to scan (eof) except for maybe a residue -> break */
        if (beg == *buflim_local)
            break;
        
        zap_nuls (beg, *buflim_local, nul_zapper);
        ...
        lim = memrchr (beg - 1, eol, *buflim_local - beg + 1);
        ++lim;
        beg[-1] = oldc;
        if (lim == beg)
            lim = beg - residue;
        beg -= residue;
        residue = *buflim_local - lim;
        ...
        i = 0;
        beg = lim;
        while (i < out_before && beg > *bufbeg_local && beg != info->lastout)
        {
            ++i;
            do
                --beg;
        ...
        if (out_byte)
            info->totalcc = add_count (info->totalcc, *buflim_local - *bufbeg_local - save);
        if (out_line)
            nlscan (beg,info);
        if (! fillbuf (save, st, info))
        {
    ...
    if (residue)
    {
        *(*buflim_local)++ = eol;
        if (info->outleft)
            nlines += grepbuf (*bufbeg_local + save - residue, *buflim_local, info);
        if (info->pending)
            prpending (*buflim_local, info);
    }
...
grepdesc (int desc, bool command_line)
{
        ...
        && directories == RECURSE_DIRECTORIES && S_ISDIR (st.st_mode))
    {
        ...
        if(!parallel){
            ...
            status_array = (bool *) malloc (sizeof (bool) * num_threads);
            locks = (pthread_mutex_t *) malloc (sizeof (pthread_mutex_t) * num_threads);
...
#endif
    if(parallel){
        static intmax_t running_index = 0;
        struct thread_routine_arg * arg =
        (struct thread_routine_arg *) malloc (sizeof (struct thread_routine_arg));
        arg->desc = desc;
        arg->filename = filename;
        arg->st = st;
        initialize_grep_info(&(arg->info));
        while(pthread_mutex_trylock (&locks[running_index]) != 0){
...
//        struct thread_routine_arg **arg_pt = &arg;
    ...
    struct grep_info info;
    initialize_grep_info(&info);
    count = grep (desc, &st, &info);
    if (count_matches)
    {
        if (out_file)
        {
...
void *
grep_thread_routine(void *arg){
    int thread_id = ((struct thread_routine_arg *)arg)->thread_id;
    int desc = ((struct thread_routine_arg *)arg)->desc;
    struct stat st = ((struct thread_routine_arg *)arg)->st;
    struct grep_info info = ((struct thread_routine_arg *)arg)->info;
    intmax_t count = grep (desc, &st, &info);
    if (count_matches)
    {
        if (out_file)
        {
            print_filename_in_thread (((struct thread_routine_arg *)arg)->filename);
            if (filename_mask)
                print_sep (SEP_CHAR_SELECTED);
            else
                putchar_errno (0);
    ...
    bool local_status = !count;
    if (list_files == (local_status ? LISTFILES_NONMATCHING : LISTFILES_MATCHING))
    {
        print_filename_in_thread (((struct thread_routine_arg *)arg)->filename);
        putchar_errno ('\n' & filename_mask);
        if (line_buffered)
    ...
    if (desc != STDIN_FILENO && close (desc) != 0)
        suppressible_error (((struct thread_routine_arg *)arg)->filename, errno);
    status_array[thread_id] &= local_status;
    free((struct thread_routine_arg *)arg);
...
main (int argc, char **argv)
{
    ...
    if(parallel && directories != RECURSE_DIRECTORIES){
