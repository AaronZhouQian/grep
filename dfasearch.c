...
static struct re_registers *regs_mthread;

void
allocate_regs (void)
{
  regs_mthread = (struct re_registers *) malloc (num_threads * sizeof (struct re_registers));
}
  ...
  for (beg = end = buf; end < buflim; beg = end)
  {
    ...
    for (i = 0; i < pcount; i++)
    {
      /* This is static because of a BRAIN-DEAD Q@#%!# library
       ...
       per-thread or the library API changed or the library
       encapsulation violated.  */
//      static struct re_registers regs[num_threads];
      
      patterns[i].not_eol = 0;
      patterns[i].newline_anchor = eolbyte == '\n';
      start = re_search (&patterns[i], beg, end - beg - 1,
                         ptr - beg, end - ptr - 1, regs_mthread + thread_id);
      if (start < -1)
        xalloc_die ();
      else if (0 <= start)
      {
        len = regs_mthread[thread_id].end[0] - start;
        match = beg + start;
        if (match > best_match)
          continue;
        if (start_ptr && !match_words)
