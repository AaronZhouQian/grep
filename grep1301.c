...
{
  uintmax_t *totalnl = &(info->totalnl);
  size_t fillsize;
  bool cc = true;
  char *readbuf;
  size_t readsize;
  ...
  while (pending > 0 && *lastout < lim)
    {
      ...
      if (outleft
          || ((execute (*lastout, nl + 1 - *lastout,
              ...
              == !out_invert))
        prline (*lastout, nl + 1, SEP_CHAR_REJECTED, info);
      else
        pending = 0;
    }
}
...
void initialize_grep_info(struct grep_info *info){
    ...
    info -> count_matches    = count_matches;
    info -> totalcc          = 0;
    info -> lastout          = 0;
  ...
  for (bool firsttime = true; ; firsttime = false)
    {
      ...
      i = 0;
      beg = lim;
      while (i < out_before && beg > bufbeg && beg != info->lastout)
        {
          ++i;
          do
            --beg;
